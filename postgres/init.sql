...
BEGIN;
CREATE OR REPLACE FUNCTION api.complete_level(p_username TEXT, p_score_increase INT, challenge INT)
RETURNS VOID AS $$
DECLARE
    new_level INT;
BEGIN
    SELECT current_level + 1 INTO new_level FROM api.users WHERE username = p_username;
	UPDATE api.users SET challenges = set_bit(challenges, challenge, 1) WHERE username = $1;
    UPDATE api.users
    SET	
